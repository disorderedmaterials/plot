...
void MainWindow::generateFunction(Test2DFunctions function)
{
    ...
    const auto axisMin = -5.0, axisMax = 5.0, delta =(axisMax - axisMin) / (nPoints_ - 1);
    ...
    switch(function)
    {
        case (Test2DFunctions::AckleyFunction):
            /*
             * Ackley's Function
             * f(x,y) = -20.0 * exp(-0.2 * sqrt(0.5 * (x^2 + y^2))) - exp(0.5 * (cos(2 * M_PI * x) + cos(2 * M_PI * y))) + e + 20
             */
            for (const auto x : x_)
                for (const auto y : y_)
                    values_.push_back(-20.0 * exp(-0.2 * sqrt(0.5 * (x*x + y*y))) - exp(0.5 * (cos(2 * M_PI * x) + cos(2 * M_PI * y))) + M_E + 20);
            break;
    }
}
